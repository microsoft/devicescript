...
* finally, functions (which are 16 bit indices) can be bound to other objects and still fit in a subnormal;
  `x.foo` where `.foo` is a function is represented as heap reference to `x` and index of `.foo` function
...
Typical embedded system is 10-100x slower than a desktop CPU, but has 100,000-1,000,000x less memory,
...
You can disassemble a compiled `.devs` or `-dbg.json` file by running `devs disasm`
(without arguments it will look in `.devicescript/bin/` where a previous `devs build`
should have left the binaries).
