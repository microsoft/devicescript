...
async function sendReq(data: any) {
    ...
    const key = sha256Hkdf(url.password, "")
    if (url.username) url.pathname += "/" + url.username
    url.username = ""
    url.password = ""
...
ENC_HTTP_URL=http://mydevice:SuPeRSecret123@localhost:8080/api/devs-enc-fetch
```
...
function getKeyForDevice(deviceId: string): Buffer | undefined {
    ...
    const deviceURL = new URL(process.env["ENC_HTTP_URL"] + "")
    if (deviceId == deviceURL.username)
        return Buffer.from(
            crypto.hkdfSync("sha256", deviceURL.password, "", "", 32)
        )
