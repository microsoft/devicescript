...
const pin = gpio(0)
// highlight-next-line
await pin.setMode(GPIOMode.Output)
```
...
await pin.write(1)
await pin.write(HIGH)
// end-highlight
```

### read

For digital input, you can use the `read` function
...
const pin = gpio(0)
await pin.setMode(GPIOMode.Input)
...
You can also use [Arduino-like `digitalWrite` functions](./wire).
