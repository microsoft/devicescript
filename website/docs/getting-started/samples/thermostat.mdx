...
Add this import to your `main.ts` file (the `@devicescript/observables` is [builtin](/developer/builtin-packages)).
...
a stream of readings and we'll use [operators](/api/observables/operators) to manipulate them.
...
temperature
    ...
    .subscribe(async level => {
        // highlight-start
        if (level < 0) await relay.active.write(true)
        else if (level > 0) await relay.active.write(false)
        console.data({ relay: await relay.active.read() })
        // highlight-end
    })
```
