---
title: Adafruit.io Temperature Feed
sidebar_position: 5
---

# Adafruit.io Temperature Feed
...
-   [Adafruit QT Py ESP32-C3 WiFi](https://www.adafruit.com/product/5405)
...
Once the flashing is done, connect to the device. If all goes well, it will appear in the device explorer.

## WiFi setup

In the DeviceScript explorer view, find the access point you plan to use and configure it with the password
(leave it empty if no password). The device should be able to connect and you'll see the IP address.

You also get the MAC address if you need it to allow access to the WiFi network.
...
for the current hardware. We will need settings to store the Adafruit.io secret key,
...
Edit `.env.local` and add your Adafruit.io secret key. **This file should not be committed to source control.**
