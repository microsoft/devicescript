...
import { trackEvent } from "@deviescript/cloud"
...
thermometer.reading
    ...
    .subscribe(async level => {
        ...
        // highlight-next-line
        if (enabled !== newEnabled) trackEvent("relay", { enabled: newEnabled })
    })
```
...
// highlight-start
const env = environment<{ t_ref }>({ t_ref: 68 })
...
const controlTemperature = () => {
    ...
    const { t_ref } = await env.read()
    unsubscribe = thermometer.reading
        .pipe(
            tap(t_raw => console.data({ t_raw })),
            ewma(0.9),
            tap(t_ewma => console.data({ t_ewma })),
